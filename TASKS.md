...
### Bug Fix: Stable Summary Sorting - Issue #39 (2025-11-24)
- [x] Investigate blinking summary lines when times are tied
- [x] Identify root cause: unstable sort when durations are equal
- [x] Fix `get_grouped_totals()` to sort by duration (desc) then task name (asc)
- [x] Add test `test_get_grouped_totals_stable_sort_on_tied_durations`
- [x] Run full test suite - all 181 tests passing
- **Context**: User reported blinking/position changes in Summary panel when tasks have identical durations
- **Root Cause**: `src/models/day_data.rs:52` used unstable sort `result.sort_by(|a, b| b.1.cmp(&a.1))` which only compared durations. When two tasks had equal durations, their order was non-deterministic across renders.
- **Solution**: Changed to stable sort with secondary key: `result.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)))` which sorts by duration (descending) then task name (ascending alphabetically)
- **Testing**: Added comprehensive test with 3 tasks of equal duration (2 hours each) named "Zebra Task", "Alpha Task", "Beta Task". Verified stable alphabetical ordering across 10 repeated calls.
- **Impact**: Summary panel now shows consistent, predictable ordering even when tasks have identical durations. No more blinking or jumping rows.
- **Files Modified**: src/models/day_data.rs (1 line fix + 27 line test)
- **Branch**: fix/issue-39-stable-summary-sort
- **Issue**: https://github.com/Kamyil/work-tuimer/issues/39
